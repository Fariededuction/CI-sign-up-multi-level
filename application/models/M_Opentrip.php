...
class M_Opentrip extends CI_Model {
  ...
  var $db;
  	var $table = "PERMOHONAN_ONLINE";

  function get_baris($id_daftar)
  {
    $sql = "
      SELECT
      ID, NAMA_DEPAN, NAMA_BELAKANG
      FROM
      DAFTAR_PERMOHONAN_WP_ONLINE
    ";
    ...
    public function getDataBerkas($email) {
    $this->db->select('*');
    $this->db->from('PERMOHONAN_ONLINE');
  //  $this->db->join('gallerygunung','gallerygunung.kode_gunung=gunung.kode_gunung');
    $this->db->where('PERMOHONAN_ONLINE.email =', $email);
    $query = $this->db->get();//disini bisa pakai tanggal != < tanggal sekarang
    return $query->result();
    }
    ...
    // public function index SplDoublyLinkedList
